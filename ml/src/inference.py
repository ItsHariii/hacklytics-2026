...
ONNX runtime inference wrapper for the RespiLens lung sound classifier.
Supports both new (5-output with class_logits) and legacy (4-output binary)
ONNX models transparently.

This module has NO PyTorch dependency — only numpy, librosa, onnxruntime.

Usage:
    from ml.src.inference import predict, load_model
...
def _decode_multilabel(crackle_logit, wheeze_logit):
    """Legacy: convert binary logits to 4-class label + confidence."""
...
def _run_inference(mel_input, tab_input, tabular, session):
    """Shared inference logic — auto-detects model format."""
    ort_inputs = {
    ...
    disease_probs = _sigmoid(disease_logits[0])
...
def _compute_feature_importance(
    ...
    Uses the primary classification output for importance scoring.
    """
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("INFERENCE MODULE TEST (4-Class)")
    print("=" * 60)
    ...
    if not model_path.exists():
        ...
        print(f"  Has class head: {_has_class_head}")
